...
subtitle: "*This post applies basic clustering methods to identify inflation regimes. It shows that headline inflation has changed regime, but not core iflation. This suggests that supply side factors dominate and it is quite likely Core inflation to change regime, too*"
date: "09/21/2021"
categories:
  - Machine learning
  - Inflation  
...
> *In this post I show how to use a basic clustering algorithm to identify different inflation regimes across time. I am comparing the regimes for both headline and core US inflation. At the time of writing, core inflation did not change regime unlike the headline inflation, which includes food and oil prices. This signals that supply side factors might be driving inflation dynamics, hence it is quite likely that core inflation might change regime too.*
...
For example, when a government announces a temporary tax hike lasting for a year this is unlikely to cool off demand much. In contrast, a tax increase that is permanent, for example as part of some reform which is expected to last for the foreseeable future, would have quite strong effects on the economy. This is an example of a permanent shock. In some cases, like now with inflation, is not very easy to make such an assessment. But, given that yields in the bond markets or gold (a traditional inflation hedge) did not move much **relative** to the size of inflation shock that we are experiencing it suggests that markets are thinking of this shock as temporary. Hence, although there is no equivalent to the example with tax announcement or an obvious regime change such as the case of Brexit where the depreciation of the Pound - against, for example, the Euro or the Dollar - was immediate and permanent, we know from the theory that temporary shocks typically trigger weaker responses.
